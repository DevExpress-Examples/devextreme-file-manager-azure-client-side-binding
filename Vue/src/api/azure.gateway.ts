  ...
  getBlobList(prefix: string): Promise<FileEntry[]> {
    return this.getAccessUrl('BlobList')
      .then((accessUrls: AccessUrls) =>
        this.executeBlobListRequest(accessUrls.url1 ?? '', prefix)
      )
      .then((xml) => {
      ...
      this.executeRequest(url)
        .then((x: AzureResponse) => {
          if (isUrlResponse(x)) {
            ...
            reject(new Error('wrong response type'));
          }
        })
      ...
      .map(
        (key) =>
          `${key}=${encodeURIComponent(
            params[key as keyof CommandParams] ?? ''
          )}`
      )
      .join('&');
  }
