...
export class AzureCustomProvider {
  ...
  getItems = (parentDirectory: FileSystemItem): Promise<FileSystemItem[]> =>
    this.azure.getItems(parentDirectory.path);

  createDirectory = (
    parentDirectory: FileSystemItem,
    name: string
  ): Promise<any> => this.azure.createDirectory(parentDirectory.path, name);

  renameItem = (item: FileSystemItem, name: string): Promise<any> =>
    item.isDirectory
      ? this.azure.renameDirectory(item.path, name)
      : this.azure.renameFile(item.path, name);

  deleteItem = (item: FileSystemItem): Promise<any> =>
    item.isDirectory
      ? this.azure.deleteDirectory(item.path)
      : this.azure.deleteFile(item.path);
  ...
  copyItem = (
    ...
    return item.isDirectory
      ? this.azure.copyDirectory(item.path, destinationPath)
      : this.azure.copyFile(item.path, destinationPath);
  };
  ...
  moveItem = (
    ...
    const destinationPath = destinationDirectory.path
      ...
      : item.name;
    return item.isDirectory
      ? this.azure.moveDirectory(item.path, destinationPath)
      : this.azure.moveFile(item.path, destinationPath);
  };
...
export function getAzureFileSystemProvider(
  endpointUrl: string,
  onRequestExecuted?: Function
): CustomFileSystemProvider {
  return new AzureCustomProvider(endpointUrl, onRequestExecuted)
    .fileSystemProvider;
}
